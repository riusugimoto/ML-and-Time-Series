...
class WeightedLeastSquares(LeastSquares):
    ...
    def fit(self, X, y, v):
        ...
        # Create the diagonal weight matrix V from the weights v
        V = np.diag(v)
...
class LeastSquaresBias:
    ...
    def predict(self, X):
        X_pred_bias = np.column_stack([np.ones(X.shape[0]), X])
        return X_pred_bias @ self.w
...
class LeastSquaresPoly:
    ...
    def predict(self, X_pred):
        """YOUR CODE HERE FOR Q3.2"""
        X_pred_bias = np.column_stack([np.ones(X_pred.shape[0]), X_pred])
        Z_pred = self._poly_basis(X_pred_bias)
