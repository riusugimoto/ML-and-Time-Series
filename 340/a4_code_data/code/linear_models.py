...
class LinearClassifierOneVsAll(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        # Initial guesses for weights: weight matrix of k rows (one for each class) and d columns (one for each feature)
        W = np.zeros([k, d])
        ...
        for i in range(k):
            print("Training the model for class {}".format(i))

            #algorithm for one vs all: the current class i is labeled as 1, and all other classes are labeled as -1
            yi = np.where(y == i, 1, -1)  
    
            #we just need wi, the optimized weights,
            # The underscores _ are placeholders indicating that the rest of the values are deliberately ignored. Referenced by chatgdp
            wi, _, _, _ = self.optimize(W[i], X, yi)  
...
class MulticlassLinearClassifier(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        # Use the SoftmaxLoss function 
        loss_fn = SoftmaxLoss()

        W_flat, _, _, _ = self.optimize(W.flatten(), X, y)

        self.W = W_flat.reshape(k, d)
